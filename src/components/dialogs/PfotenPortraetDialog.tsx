...
export function PfotenPortraetDialog({ open, onClose, onSubmit, defaultValues, recordId, besitzerList, hundeList, enablePhotoScan = true, enablePhotoLocation = true }: PfotenPortraetDialogProps) {
  ...
  const REQUIRED_FIELDS = ['besitzer', 'hund', 'titel', 'erstellungsdatum'] as const;
  ...
  async function handleAiExtract(file?: File) {
    ...
    try {
      ...
      setFields(prev => {
        ...
        const applookupKeys = new Set<string>(["besitzer", "hund"]);
  ...
  const fieldBlocks: Record<string, React.ReactNode> = {
    'besitzer': (
      <div key="besitzer" className="space-y-1.5">
        <Label htmlFor="besitzer">{fieldLabel('pfoten_portraet', 'besitzer')} <span className="text-destructive" aria-hidden="true">*</span></Label>
        <Combobox
          id="besitzer"
          placeholder="Wem gehört das Porträt?"
    ...
    'titel': (
      <div key="titel" className="space-y-1.5">
        ...
        <Input
          id="titel"
          placeholder="z. B. 'Bella – Unsere liebste Reise'"
          value={fields.titel ?? ''}
    ...
    'widmung': (
      <div key="widmung" className="space-y-1.5">
        ...
        <Textarea
          id="widmung"
          placeholder="Persönliche Widmung für die Familie..."
    ...
    'erlebnisse': (
      <div key="erlebnisse" className="space-y-1.5">
        <Label htmlFor="erlebnisse">{fieldLabel('pfoten_portraet', 'erlebnisse')}</Label>
        <Textarea
          id="erlebnisse"
          placeholder="Lustige, rührende oder besondere Momente..."
          value={fields.erlebnisse ?? ''}
          onChange={e => setFields(f => ({ ...f, erlebnisse: e.target.value }))}
          rows={3}
        />
      </div>
    ),
    'foto': (
      <div key="foto" className="space-y-1.5">
        <Label htmlFor="foto">{fieldLabel('pfoten_portraet', 'foto')}</Label>
        {fields.foto ? (
    ...
    'erstellungsdatum': (
      <div key="erstellungsdatum" className="space-y-1.5">
        <Label htmlFor="erstellungsdatum">{fieldLabel('pfoten_portraet', 'erstellungsdatum')} <span className="text-destructive" aria-hidden="true">*</span></Label>
        <DatePicker
          id="erstellungsdatum"
  ...
  const FIELD_LABELS: Record<string, string> = {"besitzer": "Besitzer", "hund": "Hund", "titel": "Titel des Porträts", "widmung": "Persönliche Widmung", "erlebnisse": "Besondere Erlebnisse & Erinnerungen", "foto": "Foto des Hundes", "erstellungsdatum": "Erstellungsdatum"};
  const CURRENCY_KEYS = new Set<string>([]);
  // Applookup-Referenz-Labels: pro applookup-Feld in dieser Form (ownKey)
  // eine Map { lookupKey: label } für ALLE Felder des Target-Schemas. Wird
  // beim Render-Walk gefiltert auf die in der computed-Formel tatsächlich
  // referenzierten lookupKeys (siehe applookupRefs unten).
  const APPLOOKUP_LABELS: Record<string, Record<string, string>> = {"besitzer": {"vorname": "Vorname", "nachname": "Nachname", "telefon": "Telefonnummer", "email": "E-Mail-Adresse", "strasse": "Straße", "hausnummer": "Hausnummer", "plz": "Postleitzahl", "ort": "Ort", "notizen": "Notizen"}, "hund": {"name": "Name des Hundes", "rasse": "Rasse", "geburtsdatum": "Geburtsdatum", "geschlecht": "Geschlecht", "gewicht_kg": "Gewicht (kg)", "kastriert": "Kastriert / Sterilisiert", "impfstatus": "Impfstatus", "gesundheitshinweise": "Gesundheitshinweise & Besonderheiten", "tierarzt": "Tierarzt (Name & Telefon)", "besitzer": "Besitzer"}};
