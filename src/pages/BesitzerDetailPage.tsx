...
import { formEnhancements } from '@/config/form-enhancements/Besitzer';
import { evalComputed } from '@/config/form-enhancements/types';
import { t, appLabel, fieldLabel, localeTag, CURRENCY } from '@/i18n';

export default function BesitzerDetailPage() {
  const { id } = useParams<{ id: string }>();
  const navigate = useNavigate();
  const [record, setRecord] = useState<Besitzer | null>(null);
  ...
  async function loadData() {
    setLoading(true);
    try {
      const list = await LivingAppsService.getBesitzer();
  ...
  async function handleUpdate(fields: Besitzer['fields']) {
    if (!record) return;
    await LivingAppsService.updateBesitzerEntry(record.record_id, fields);
    await loadData();
    setEditing(false);
  }
  ...
  async function handleDelete() {
    if (!record) return;
    await LivingAppsService.deleteBesitzerEntry(record.record_id);
    setDeleteOpen(false);
    navigate('/besitzer');
  }
  ...
  if (!record) {
    return (
      <RecordViewEmpty
        title={t('not_found')}
        action={
          <Button variant="ghost" onClick={() => navigate('/besitzer')}>
  ...
  return (
    <RecordView
      onBack={() => navigate('/besitzer')}
      onEdit={() => setEditing(true)}
      backLabel={t('back')}
      editLabel={t('edit_button')}
    >
      <RecordHeader title={record.fields.vorname ?? appLabel('besitzer')} />
